...
function EmailVerify() {
  ...
  useEffect(() => {
    if (!email || !password) {
      toast.error("Email or password missing. Please login again.");
      navigate("/login");
    }
  ...
  const handleVerify = async () => {
    ...
    if (otp.length !== 6) return toast.error("Please enter all 6 digits of OTP");
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(90deg,#6a5af9,#8289f9)' }}
    >
      <Link
        to="/"
